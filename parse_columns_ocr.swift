...
func parseImage(imagePath: String, fileIndex: Int) {
    ...
    // Find column split: typically near x ~ 0.50
...
let files = FileManager.default.enumerator(atPath: "raw_biology_questions")?.allObjects as? [String] ?? []
let imageFiles = files.filter { $0.hasSuffix(".jpeg") || $0.hasSuffix(".jpg") || $0.hasSuffix(".png") }.sorted()

for (idx, file) in imageFiles.enumerated() {
    parseImage(imagePath: "raw_biology_questions/\(file)", fileIndex: idx + 1)
}
