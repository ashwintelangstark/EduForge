...
let files = FileManager.default.enumerator(atPath: "raw_physics_questions")?.allObjects as? [String] ?? []
let imageFiles = files.filter { $0.hasSuffix(".jpeg") || $0.hasSuffix(".jpg") || $0.hasSuffix(".png") }.sorted()

for (idx, file) in imageFiles.enumerated() {
    parseImage(imagePath: "raw_physics_questions/\(file)", fileIndex: idx + 1)
}
